...
const iconesPredefinidos = [
  ...
  { value: 'tool', label: '🔧 Manutenção' },
  { value: 'dollar-sign', label: '💰 Investimentos' },
  { value: 'credit-card', label: '💳 Cartão' },
  { value: 'file-text', label: '📄 Impostos' },
  { value: 'users', label: '👥 Pessoal' },
    ...
    >
      ...
      <div className="space-y-4">
        ...
        >
          <input
            ...
            placeholder="Ex: Alimentação, Transporte, Vendas..."
          ...
          <div className="flex gap-4">
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                name="tipo"
                value="despesa"
                checked={form.values.tipo === 'despesa'}
                onChange={() => form.setFieldValue('tipo', 'despesa')}
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                name="tipo"
                value="receita"
                checked={form.values.tipo === 'receita'}
                onChange={() => form.setFieldValue('tipo', 'receita')}
          ...
          <div className="space-y-3">
            <div className="flex flex-wrap gap-2">
              {coresPredefinidas.map((cor) => (
                <button
                  key={cor.value}
                  type="button"
                  onClick={() => form.setFieldValue('cor', cor.value)}
                  className={cn(
                    'w-8 h-8 rounded-full border-2 transition-transform hover:scale-110',
                    form.values.cor === cor.value
            ...
            <div className="flex items-center gap-3">
              <input
                ...
                onChange={(e) => form.setFieldValue('cor', e.target.value)}
                className="w-10 h-10 p-1 border border-gray-300 rounded cursor-pointer dark:border-gray-600"
              />
              <input
                type="text"
                value={form.values.cor}
                onChange={(e) => form.setFieldValue('cor', e.target.value)}
                placeholder="#000000"
                className="w-28 px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-gray-800 dark:border-gray-600 dark:text-white"
              />
              <div
                className="flex items-center justify-center w-10 h-10 rounded-lg text-white font-bold text-sm"
                style={{ backgroundColor: form.values.cor }}
              >
          ...
          <div className="grid grid-cols-4 sm:grid-cols-8 gap-2">
            {iconesPredefinidos.map((icone) => (
              <button
                key={icone.value}
                type="button"
                onClick={() => form.setFieldValue('icone', icone.value)}
                className={cn(
                  'p-2 text-xl rounded-lg border-2 transition-colors hover:bg-gray-100 dark:hover:bg-gray-700',
                  form.values.icone === icone.value
      ...
      {categoriaPai.length > 0 && (
        <div className="space-y-4">
          ...
          <FormField
            label="Categoria Pai"
            error={form.errors.categoriaPaiId}
            hint="Opcional - selecione para criar uma subcategoria"
          >
            <select
              {...form.getFieldProps('categoriaPaiId')}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-gray-800 dark:border-gray-600 dark:text-white"
            >
              <option value="">Nenhuma (categoria principal)</option>
              {categoriaPai.map((cat) => (
          ...
          >
            {form.values.icone ? (
              <span className="text-lg">
                {iconesPredefinidos.find((i) => i.value === form.values.icone)?.label.split(' ')[0] || '⭐'}
              </span>
            ) : (
              <span className="font-bold">
                {form.values.nome?.charAt(0)?.toUpperCase() || 'C'}
              </span>
            )}
          </div>
          <div>
            <p className="font-medium text-gray-900 dark:text-white">
              {form.values.nome || 'Nome da categoria'}
            </p>
