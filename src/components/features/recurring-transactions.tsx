...
const statusConfig: Record<RecurrenceStatus, {
  label: string;
  color: string;
  bgColor: string;
    ...
    <div className={cn('', className)}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="p-4 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg">
          <p className="text-sm text-gray-500 dark:text-gray-400">Recorrências Ativas</p>
        ...
        <div className="p-4 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg">
          <p className="text-sm text-gray-500 dark:text-gray-400">Receitas Mensais</p>
...
function RecurringTransactionCard({
  ...
  return (
    <div
      className={cn(
        'bg-white dark:bg-gray-800 border rounded-lg overflow-hidden transition-colors',
        isOverdue 
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              ...
              {isOverdue && (
                <span className="px-2 py-0.5 text-xs font-medium bg-red-50 dark:bg-red-900/20 text-red-600 rounded-full">
                  Atrasado
                </span>
              )}
            </div>
            <div className="flex items-center gap-4 text-sm text-gray-500 dark:text-gray-400">
              ...
              <span className="flex items-center gap-1">
                ...
                Próxima: {format(transaction.nextOccurrence, 'dd/MM/yyyy', { locale: ptBR })}
              </span>
            </div>
          </div>
        ...
        <div className="px-4 pb-4 border-t border-gray-200 dark:border-gray-700">
          <div className="pt-4 grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div>
              ...
              <dd className="text-gray-900 dark:text-white">
                {format(transaction.startDate, 'dd/MM/yyyy', { locale: ptBR })}
              </dd>
            </div>
            {transaction.endDate && (
              <div>
                ...
                <dd className="text-gray-900 dark:text-white">
                  {format(transaction.endDate, 'dd/MM/yyyy', { locale: ptBR })}
                </dd>
              </div>
            )}
            <div>
              <dt className="text-gray-500 dark:text-gray-400">Ocorrências</dt>
              <dd className="text-gray-900 dark:text-white">
                {transaction.completedOccurrences}
                {transaction.totalOccurrences ? ` / ${transaction.totalOccurrences}` : ''}
              </dd>
            </div>
            {transaction.lastProcessedAt && (
              <div>
                <dt className="text-gray-500 dark:text-gray-400">Última execução</dt>
                <dd className="text-gray-900 dark:text-white">
                  {format(transaction.lastProcessedAt, 'dd/MM/yyyy', { locale: ptBR })}
                </dd>
              </div>
            )}
            {transaction.supplier && (
              <div>
                <dt className="text-gray-500 dark:text-gray-400">Fornecedor</dt>
                <dd className="text-gray-900 dark:text-white">{transaction.supplier}</dd>
              </div>
            )}
            {transaction.customer && (
              <div>
                <dt className="text-gray-500 dark:text-gray-400">Cliente</dt>
                <dd className="text-gray-900 dark:text-white">{transaction.customer}</dd>
              </div>
            )}
          </div>
          ...
          {transaction.notes && (
            <div className="mt-4 text-sm">
              ...
              <dd className="text-gray-900 dark:text-white">{transaction.notes}</dd>
            </div>
          )}
...
function getRecurrenceText(transaction: RecurringTransaction): string {
  ...
  if (recurrenceInterval === 1) {
    return recurrenceLabels[recurrenceType];
  }
  
  const labels: Record<RecurrenceType, string> = {
    daily: `A cada ${recurrenceInterval} dias`,
    weekly: `A cada ${recurrenceInterval} semanas`,
    monthly: `A cada ${recurrenceInterval} meses`,
    yearly: `A cada ${recurrenceInterval} anos`,
  };
...
function getMonthlyAmount(transaction: RecurringTransaction): number {
  ...
  switch (recurrenceType) {
    case 'daily':
      return (amount / recurrenceInterval) * 30;
    case 'weekly':
      return (amount / recurrenceInterval) * 4;
    case 'monthly':
      return amount / recurrenceInterval;
    case 'yearly':
      return (amount / recurrenceInterval) / 12;
    default:
      return amount;
  }
}
